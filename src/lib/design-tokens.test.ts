...
function collectLeafValues(node: unknown, prefix: string[], out: string[]): void {
...
function collectResolvedLeafValues(node: unknown, prefix: string[], lookup: Map<string, string>, out: string[]): void {
	if (typeof node !== "object" || node === null) return;
	const obj = node as DtcgNode;
	if ("$value" in obj) {
		const raw = obj.$value;
...
describe("design-tokens enums", () => {
	...
	test("BrandColor covers the brand palette", () => {
		...
		const expected = Object.keys(brand).filter((k) => !k.startsWith("$")).length;
	...
	test("SemanticColor covers every semantic role", () => {
		...
		const expected = Object.keys(semantic).filter((k) => !k.startsWith("$")).length;
	...
	test("StatusColor covers status colors", () => {
		...
		const expected = Object.keys(status).filter((k) => !k.startsWith("$")).length;
	...
	test("Spacing covers 1..16", () => {
		...
		const expected = Object.keys(spacing).filter((k) => !k.startsWith("$")).length;
	...
	test("resolved Color values agree with the JSON after reference expansion", () => {
		...
		const resolve = (raw: string): string => {
			const lookup = new Map(brandValues.map((v, i) => [Object.keys(brandGroup).filter((k) => !k.startsWith("$"))[i], v]));
			let value = raw;
			let prev = "";
			while (prev !== value) {
				prev = value;
				value = value.replace(/\{([^}]+)\}/g, (_m, refName: string) => {
					return lookup.get(refName.trim().replace(/^color\.brand\./, "")) ?? _m;
				});
			}
			return value;
		};

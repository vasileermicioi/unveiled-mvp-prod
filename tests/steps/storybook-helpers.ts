...
export const STORY_TAG = /^@story\((?<body>[^)]+)\)$/;
...
export function findStoryTag(scenario: ParsedScenario): string | undefined {
  return scenario.tags.find((tag) => tag.startsWith("@story"));
}
...
export function parseStoryBody(body: string): {
  ...
  const match = /component=(?<component>[^,]+),\s*story=(?<story>[^,\s)]+)/.exec(
    body,
  );
...
export function storyUrlFor(
  ...
  return `${normalized}/?path=/story/${id}&id=${id}`;
}

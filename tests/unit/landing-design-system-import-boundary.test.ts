...
const FORBIDDEN_PATTERNS: Array<{ pattern: RegExp; reason: string }> = [
  ...
  },
  {
    ...
    reason: "landing package must not import from local components/landing/",
  },
];
...
const files = walk(LANDING_SRC, extensions);
...
if (failures.length > 0) {
  console.error("landing-design-system-import-boundary failed:");
...
console.log(
  `landing-design-system-import-boundary passed (${files.length} files)`,
);

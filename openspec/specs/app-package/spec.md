...
#### Scenario: No @/ imports remain in production code

- **WHEN** `bun run check` runs the umbrella lint + typecheck
- **THEN** the codemod verification step asserts that no file under `packages/app/src/**` contains a `@/` import

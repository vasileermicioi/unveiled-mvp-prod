...
### Requirement: C4 Model Path Verification

The LikeC4 model's referenced file paths (`metadata.path`) SHALL be verified by unit tests under `tests/architecture/` to ensure they point to real files or directories inside the repository, preventing regression to pre-monorepo paths.

#### Scenario: Unit tests validate model paths

- **WHEN** `bun run test:unit` is executed
- **THEN** unit tests read the exported LikeC4 model elements
- **AND** assert that at least one `metadata.path` is non-empty, exists on disk, and is located within the workspace.

...
A scenario MAY carry a `@story(component=…, story=…)` tag. When the tag is present, the runner SHALL open the Storybook iframe at the corresponding story URL and run the same step dispatcher inside it. When the tag is absent, the runner SHALL run the scenario against the real route.

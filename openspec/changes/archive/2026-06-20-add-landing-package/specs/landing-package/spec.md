# Capability: landing-package

## Purpose

Define `@unveiled/landing`, the Astro 6 SSR application that owns the public landing surface (`/*` in production) for unauthenticated visitors. The package reuses `@unveiled/design-system` for primitives, owns the brand chrome (header, hero, footer), the marketing CTA into `/app/*`, and ships the gherkin + Ladle scaffolding so richer landing content can land in follow-up iterations without restructuring.
...
- **AND** the same global `@media (prefers-reduced-motion: reduce)` block in `packages/landing/src/styles/global.css` governs the behavior (no per-island `useReducedMotion()` hook).

### Requirement: `@unveiled/landing` has its own global stylesheet

The package MUST ship `packages/landing/src/styles/global.css` which imports the design-system tokens (`@unveiled/design-system/styles/generated/tokens.css`) and the Tailwind v4 layers. The stylesheet MUST include the single global `@media (prefers-reduced-motion: reduce)` block that governs the landing surface.

#### Scenario: Stylesheet imports tokens through the design system

- **WHEN** `packages/landing/src/styles/global.css` is read
- **THEN** it contains an `@import` for `@unveiled/design-system/styles/generated/tokens.css`
- **AND** it applies the Tailwind v4 layers.

#### Scenario: Motion guard is a single global block

- **WHEN** the stylesheet is audited
- **THEN** every landing motion surface is covered by a single `@media (prefers-reduced-motion: reduce)` block in `packages/landing/src/styles/global.css`

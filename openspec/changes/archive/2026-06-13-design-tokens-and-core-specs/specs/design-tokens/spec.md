...
A committed `src/styles/generated/tokens.css` file SHALL be generated from `design-tokens.json` and imported from `src/styles/global.css`.

#### Scenario: Generation script emits CSS custom properties
- **WHEN** `bun run tokens:gen` runs
- **THEN** it reads `design-tokens.json` and writes `src/styles/generated/tokens.css`
...
- **WHEN** `src/styles/global.css` is loaded
...
- **THEN** `src/styles/generated/tokens.css` is present without needing to run the generator
- **AND** the file is excluded from Biome formatting in `biome.json`

### Requirement: Typed TypeScript Enums Expose the Tokens
A `src/lib/design-tokens.ts` module SHALL re-export the token groups as `as const` objects and derived types so variant code references tokens by enum member, not magic string.

#### Scenario: Enum module exports typed groups
- **WHEN** a contributor imports from `src/lib/design-tokens.ts`
...
- **THEN** every variant in `src/components/ui/unveiled-primitives.tsx` that referenced the old name via the enum fails TypeScript compilation
...
- **WHEN** a new button, badge, or panel variant is added to `unveiled-primitives.tsx`
- **THEN** its color, radius, shadow, and spacing values reference a typed enum member from `src/lib/design-tokens.ts`
...
A `bun run tokens:check` script SHALL fail with a non-zero exit code when the generated `src/styles/generated/tokens.css` is out of sync with `design-tokens.json`.
...
- **AND** if the generated CSS disagrees with the JSON, `bun run check` exits non-zero with a message naming the drifted file
...
- **THEN** the generated CSS is updated

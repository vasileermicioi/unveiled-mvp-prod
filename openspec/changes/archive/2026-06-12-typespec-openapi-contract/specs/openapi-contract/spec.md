...
Define a TypeSpec-authored OpenAPI 3.1 contract as the single source of truth for the HTTP and Astro Action surfaces exposed by the app, and codify the drift rules that keep the contract in sync with the source.
...
A `specs:check` script SHALL detect drift between the TypeSpec source and the committed emitter output, and SHALL be wired into `bun run check`.
...
- **AND** the failure message points at the drifted file (`openapi.yaml` or `request-validators.ts`)
...
#### Scenario: Adding a new route requires updating the contract

- **WHEN** a developer adds a new route under `src/pages/api/**`
- **THEN** adding the corresponding TypeSpec operation is a hard prerequisite for the route to ship
- **AND** the contract test under `specs:check` would fail if the route is added without a matching TypeSpec model

### Requirement: OpenAPI Document Is Served At A Stable URL

A stable URL SHALL serve the generated OpenAPI document so auditors, partner integrators, and downstream codegen can fetch the contract.

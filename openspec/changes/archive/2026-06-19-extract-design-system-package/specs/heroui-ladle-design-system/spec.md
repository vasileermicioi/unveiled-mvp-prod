...
The system MUST ship `packages/design-system/src/heroui-replica/design-system-overview.ladle.tsx`, which renders a single Ladle page laid out in a `<main role="main">` element with an `<h1>` heading "Unveiled Design System (HeroUI)" and a `<nav>` of anchor links, where each anchor link navigates to a section that renders a `Hero<Name>` primitive from the replica.

#### Scenario: The overview file is missing

- **WHEN** `packages/design-system/src/heroui-replica/design-system-overview.ladle.tsx` does not exist
- **THEN** `bun --filter @unveiled/design-system run heroui-design-system-replica:check` MUST fail.
...
- **THEN** `bun --filter @unveiled/design-system run heroui-design-system-replica:check` MUST fail and identify the missing landmark.
...
- **THEN** `bun --filter @unveiled/design-system run heroui-design-system-replica:check` MUST fail and name the omitted primitive.
...
## Purpose
Define the Cloudflare Worker entry that owns the public URL surface (`/*`, `/app/*`, `/api/*`, `/healthz`, `/readyz`) and dispatches every inbound request to the right downstream Worker (landing Astro app, app Astro app, API Worker) via Cloudflare service bindings. The orchestrator also owns uniform security headers (CSP, HSTS, X-Content-Type-Options), structured JSON request logging with a `requestId` correlation id, and the public liveness / readiness probes that replace the previous `/api/health.json` and `/api/readiness.json` endpoints.

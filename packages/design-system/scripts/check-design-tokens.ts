...
const TARGETS = [
  join(REPO_ROOT, "packages/design-system/src/styles/generated/tokens.css"),
  join(REPO_ROOT, "packages/design-system/src/lib/design-tokens.types.ts"),
];
...
function regen(): string {
  const result = spawnSync(
    ...
    {
      cwd: join(REPO_ROOT, "packages/design-system"),
      encoding: "utf8",
    },
  );
  if (result.status !== 0) {
